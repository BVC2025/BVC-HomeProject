...
export default function Payroll() {
  ...
  // Load existing run + slips for the month
  const loadRun = async () => {
    setLoading(true);
    try {
  ...
  // Fetch monthly attendance for all employees in parallel
  useEffect(() => {
  ...
  // Build per-employee summary using HR rules
  const allRows = useMemo(() => {
    ...
    return employees.map((e) => {
      ...
      return {
        emp: e, slip, base, perDay,
        present: att.present, absent: att.absent, late: att.late,
        clTotal, clPaid, clUnpaidDays,
        permTotalH, permPaidH, permUnpaidH, permUnpaidDays,
        unpaidDays, deduction, increment, net,
  ...
  const runPayroll = async () => {
    ...
    try {
      await API.post("/payroll/generate", {
        ...
        INCREMENTS_BY_EMPLOYEE: increments,
      });
      await loadRun();
    } catch (err) {
      ...
      <div className={styles.modalPanel} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.summaryGrid}>
            ...
            <SummaryRow label="Deduction (unpaid days)"     value={`− ₹ ${inr(r.deduction)}`} bad />
            <SummaryRow label="Increment (HR selected)"     value={`+ ₹ ${inr(r.increment)}`} good={r.increment > 0} />
          </div>
