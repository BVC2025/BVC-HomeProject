...
def create_invitation(payload: InvitationCreate, db: Session = Depends(get_db)):
    ...
    if payload.INVITED_EMAIL:
        ...
        logo_bytes, logo_content_type, logo_html = build_email_logo(company)
...
def approve_invitation(
    ...
    try:
        ...
        _logo_bytes, _logo_ct, _logo_html = build_email_logo(_company)
        ...
        _rendered_html = apply_cid_logo(_rendered_html, _logo_bytes, _company)
        if not _logo_bytes:
            _rendered_html, _logo_bytes, _logo_ct = extract_cid_logo(_rendered_html)
