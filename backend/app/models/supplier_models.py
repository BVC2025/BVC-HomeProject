...
# Supplier master lives in models.py (`class Supplier`) — that's the
# actively-used definition app-wide. This module used to carry a second,
# colliding `class Supplier(Base)` (same tablename "supplier"), which
# SQLAlchemy rejects at mapper-configuration time. Removed; nothing in
# this file needs its own copy since SQLAlchemy resolves "Supplier"
# string relationship targets against the shared registry regardless of
# which module the class is physically defined in.
